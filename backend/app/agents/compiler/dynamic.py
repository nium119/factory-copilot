    ...
    async def _plan_steps(self, message: str, history_messages: list) -> tuple:
        ...
        try:
            ...
            plan_instruction = (
                ...
                "- 用户消息已含明确对象/编码（如 ECN2026-002、MO001），或明确的全量/列表/统计/概览意图（如「所有工单」「库存概览」「本月异常统计」），或对话历史已含对象时，必须直接规划，禁止 ask\n"
                "- 意图明确但缺少定位具体对象的标识时（如「工单的BOM变更有什么影响」没给工单号、查询链需要定位到具体对象），必须输出 ask 向用户要对象标识（如\"请问要分析哪个工单？请提供工单编号（如 MO001）\"），options 可给「列出所有工单供我选择」这类引导选项；严禁不问就全表查询后拿任意一条记录继续分析\n"
